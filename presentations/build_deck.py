...
OUT = Path(__file__).parent / "AI-SDLC-team-presentation.pptx"
...
chip_y = agt_y + Inches(0.95)
chip_h = Inches(0.36)
chip_gap = Inches(0.1)
cx_cursor = Inches(0.55)
char_w = 0.075
for label, color, dim in agents:
    chip_w = Inches(0.45 + char_w * len(label))
    add_chip_dot(s, cx_cursor, chip_y, chip_w, chip_h, label,
...
# =========================================================================
# Slide 11 — UI surface (claudboard mockup styling)
# =========================================================================
s = blank_slide()
...
# Sidebar (.side)
sb_x = bx; sb_y = by + Inches(0.32)
sb_w = Inches(1.6); sb_h = bh - Inches(0.32)
add_rect(s, sb_x, sb_y, sb_w, sb_h, BG_2)
add_rect(s, sb_x + sb_w, sb_y, Emu(6000), sb_h, BORDER)
# brand inside
add_round(s, sb_x + Inches(0.15), sb_y + Inches(0.15),
          Inches(0.22), Inches(0.22), TEXT, radius=0.2)
add_text(s, sb_x + Inches(0.15), sb_y + Inches(0.15),
         Inches(0.22), Inches(0.22), "cb",
         size=8, bold=True, color=BG, font=FONT_MONO,
         align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
add_text(s, sb_x + Inches(0.42), sb_y + Inches(0.14),
         Inches(1), Inches(0.25), "claudboard",
         size=10, bold=True, color=TEXT)
# divider
add_rect(s, sb_x + Inches(0.1), sb_y + Inches(0.5),
         sb_w - Inches(0.2), Emu(6000), BORDER)
# nav label
add_text(s, sb_x + Inches(0.15), sb_y + Inches(0.6),
         sb_w - Inches(0.2), Inches(0.2), "WORKSPACE",
         size=8, bold=True, color=DIM, font=FONT_SANS)
# nav items
nav_items = [("Dashboard", False),
             ...
             ("Review gate", True),
             ("History", False)]
for i, (lbl, active) in enumerate(nav_items):
    iy = sb_y + Inches(0.85 + i * 0.32)
    if active:
        add_round(s, sb_x + Inches(0.1), iy,
                  sb_w - Inches(0.2), Inches(0.28),
                  SURFACE_2, radius=0.15)
        add_rect(s, sb_x + Inches(0.1), iy,
                 Emu(8000), Inches(0.28), AMBER)
    add_text(s, sb_x + Inches(0.22), iy,
             sb_w - Inches(0.3), Inches(0.28), lbl,
             size=10, color=TEXT if active else TEXT_2,
             bold=active, anchor=MSO_ANCHOR.MIDDLE)

# Main pane
pane_x = sb_x + sb_w + Inches(0.2)
pane_y = sb_y + Inches(0.2)
pane_w = bw - sb_w - Inches(0.4)

# topbar / crumb
add_text(s, pane_x, pane_y, pane_w, Inches(0.25),
         "Projects  ›  platform  ›  PLAT-42117",
         size=10, color=MUTED, font=FONT_MONO)
# Title
add_text(s, pane_x, pane_y + Inches(0.3), pane_w, Inches(0.4),
         "Add export-as-CSV to user list",
         size=15, bold=True, color=TEXT)
# Status chip (amber gate)
add_round(s, pane_x, pane_y + Inches(0.72),
          Inches(1.6), Inches(0.3), AMBER_DIM, radius=0.5)
add_oval(s, pane_x + Inches(0.15), pane_y + Inches(0.81),
         Inches(0.1), Inches(0.1), AMBER)
add_text(s, pane_x + Inches(0.3), pane_y + Inches(0.72),
         Inches(1.3), Inches(0.3), "paused-gate",
         size=9, bold=True, color=AMBER, font=FONT_MONO,
         anchor=MSO_ANCHOR.MIDDLE)

# Mini phase pipeline (matches .phase pattern)
mp_y = pane_y + Inches(1.2)
mp_diam = Inches(0.42)
n = 7
mp_spacing = (pane_w - mp_diam) / (n - 1)
status = ["done", "done", "active", "pending", "pending", "pending", "pending"]
status_color = {
    "done":    (GREEN, GREEN_DIM),
    "active":  (AMBER, AMBER_DIM),
    "pending": (MUTED, SURFACE_2),
}
add_rect(s, pane_x + mp_diam / 2, mp_y + mp_diam / 2,
         mp_spacing * (n - 1), Emu(6000), BORDER_HI)
for i in range(n):
    cx = pane_x + mp_spacing * i
    color, dim = status_color[status[i]]
    add_oval(s, cx, mp_y, mp_diam, mp_diam, dim)
    add_text(s, cx, mp_y, mp_diam, mp_diam,
             str(i + 1), size=11, bold=True,
             color=color, font=FONT_MONO,
             align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)

# Gate review card (matches .gate-head + .gate-col pattern)
g_y = mp_y + Inches(0.8)
g_h = Inches(2.0)
add_card(s, pane_x, g_y, pane_w, g_h)
# head with amber bg accent
add_round(s, pane_x, g_y, pane_w, Inches(0.4), AMBER_DIM, radius=0.04)
add_rect(s, pane_x, g_y + Inches(0.2), pane_w, Inches(0.2),
         RGBColor(0x1A, 0x16, 0x10))  # subtle fade
add_oval(s, pane_x + Inches(0.15), g_y + Inches(0.12),
         Inches(0.18), Inches(0.18), AMBER)
add_text(s, pane_x + Inches(0.4), g_y + Inches(0.05),
         pane_w - Inches(0.5), Inches(0.3),
         "Review gate  ·  spec + plan",
         size=11, bold=True, color=TEXT)
# Path
add_text(s, pane_x + Inches(0.2), g_y + Inches(0.5),
         pane_w - Inches(0.3), Inches(0.3),
         "specs/001-PLAT-42117-export-csv/",
         size=10, color=TEXT_2, font=FONT_MONO)
# file chips
fp_y = g_y + Inches(0.9)
file_chips = ["business-behavior-spec.md", "authorization-spec.md",
              "execution-plan.md"]
cx = pane_x + Inches(0.2)
for label in file_chips:
    w = Inches(1.7)
    add_chip(s, cx, fp_y, w, Inches(0.3), label,
             fill=SURFACE_2, fg=TEXT_2, border=BORDER_HI,
             size=9, mono=True)
    cx += w + Inches(0.1)
# buttons (matches .btn.amber + .btn)
btn_y = g_y + g_h - Inches(0.5)
add_round(s, pane_x + Inches(0.2), btn_y,
          Inches(1.0), Inches(0.32), AMBER, radius=0.18)
add_text(s, pane_x + Inches(0.2), btn_y, Inches(1.0), Inches(0.32),
         "Approve", size=10, bold=True, color=ON_AMBER,
         align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
add_round(s, pane_x + Inches(1.3), btn_y,
          Inches(1.4), Inches(0.32),
          SURFACE, line=BORDER_HI, line_w=0.5, radius=0.18)
add_text(s, pane_x + Inches(1.3), btn_y, Inches(1.4), Inches(0.32),
         "Request changes", size=10, color=TEXT,
         align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
